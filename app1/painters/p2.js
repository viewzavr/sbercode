...
export function setup( vz ) {
  vz.addItemType( "zoo-p2","ZOO: круговой график P2", function( opts ) {
...
//  opts.name ||= "simple-graph-P1";
  ...
  var trafunc = new Function( "pt","return pt" );
  ...
  function qq() {
    ...
    for (var i=0; i<v.length; i++) {
      ...
      var angle = ((1.0*x)/xmax)*Math.PI*2;
      x = Math.sin(angle)*r;
      z = Math.cos(angle)*r;
      
      coords.push( sx*x, sy*y,sx*z );
    ...
    if (obj.getParam("lines") && lines) {
      var coords2 = []; // new Float32Array( v.length );
      for (var i=0; i<v.length; i++) 
        coords2.push( coords[3*i],coords[3*i+1],coords[3*i+2],0,0,0 );
      lines.positions = coords2;
    }
